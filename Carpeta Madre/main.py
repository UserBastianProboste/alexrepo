import pymongo
import GenealogiaChile_Setup_Pack
import ProyectoGenealogiaChile
import genealogia_chile_output

class GenealogiaDB:
    """Simple wrapper for a MongoDB collection storing familias."""
    ...
    def insertar_familia(self, familia):
        """Insert a family document into the collection."""
        return self.collection.insert_one(familia)

    def obtener_familias(self):
        """Return all stored families."""
...
def imprimir_arbol(personas_dict, nodos, nivel=0):
    """Display the genealogical tree in a readable format."""
    for nodo in nodos:
        persona = personas_dict[nodo]
        indent = "    " * nivel
        print(
            f"{indent}- {persona['nombre']} ({persona['familia']}, {persona['siglo']}) - {persona['rol']}"
        )
        if persona["hijos"]:
...
def mostrar_por_familia(familia):
    """Print people belonging to a given family."""
...
def mostrar_por_siglo(siglo):
    """Print people that lived in the given century."""
    personas = obtener_personas()
...
def buscar_por_nombre(nombre):
    """Search for people containing the provided name."""
    personas = obtener_personas()
...
def menu_interactivo():
    """Mostrar un menú sencillo para consultar la genealogía."""
    while True:
        print("\n====== MENÚ GENEALÓGICO ======")
